...
/* 
25.10.09
added by Nikita Eremin
mailto: nikitaeremin@gmail.com 
*/
...
(function(){
    ...
    jQuery.fn.css = function( key, value ){
        if (key.rotate){
            var rotatePic = $(this.rotate( key.rotate ));
            return this;
        }
        return oldCss.call( this, key, value );
    };
    
    var oldFxStepDefault = jQuery.fx.step._default;
    
    jQuery.fx.step._default = function( fx ){
        if ( fx.prop == 'rotate'){
            if (! fx.elem.id) fx.elem.id = 'rotatepic' + now();
            var elem = $('#' + fx.elem.id ); // doesn't work for $(fx.elem)
            elem.css({ rotate : parseInt(fx.now)})
        }
        return oldFxStepDefault.call( this, fx);
    };
    
    /* TODO : make animate({ rotate : '+=70'}) work
    var oldCustom = jQuery.fx.prototype.custom;
    
    jQuery.fx.prototype.custom = function( from, to, unit ){
        if ( $(this.elem).attr('rotate') != undefined) {
            from += $(this.elem).attr('rotate');
        }
        return oldCustom.call( this, from, to, unit );
    };    
    */
...
$(document).ready( function(){
    $('img[rotate]').each(function(){
        var img = $(this);
        img.rotate(parseInt(img.attr('rotate')))
    });  
});    