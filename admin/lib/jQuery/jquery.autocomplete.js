	...
	return this.each( function()//do it for each matched element
	{
		...
		function getData(text)
		{
			...
			if (text != oldText && (settings.minChars != null && text.length >= settings.minChars))
			{
				...
				$j.getJSON(url, settings.parameters, function(data)
				{
					var items = '';
					if (data)
					{
						...
						for (i = 0; i < data.length; i++)//iterate over all options
						{
						  ...
						  list.show().children().
						  hover(function() { $j(this).addClass("selected").siblings().removeClass("selected");}, function() { $j(this).removeClass("selected") } ).
						  click(function () { valueInput.val( $j(this).attr('value') );textInput.val( $j(this).text() ); clear(); });
						}
		...
		textInput.keydown(function(e) 
		{
			...
			{ 
				if (typeof settings.onClick == "function") {
					settings.onClick(valueInput.val(), textInput.val());
				}
