var _GAttrEx = function(pEl, atrName, atrNamePHP,pTaskbar)
{
	var returnPHP_atr = function(str, b1)
	{
		return '<?'+JS_stripslashes(pTaskbar.pMainObj.arAPPFragments[parseInt(b1)])+'?>';
	}
	var v = GAttr(pEl, atrNamePHP);
	if (pTaskbar.pMainObj.bUseAPP && v.length > 0)
		return  v.replace(/#APP(\d+)#/ig, returnPHP_atr);

	return GAttr(pEl, atrName);
}

var _SAttrEx = function(pEl,atrName,atrNamePHP,val,pTaskbar)
{
	if (pTaskbar.pMainObj.bUseAPP && val.substr(0,2) == '<?' && val.substr(val.length-2,2) == '?>')
	{
		var rep = function(str,b1)
		{
			var v = GAttr(pEl, atrNamePHP), i;
			if (v.length > 0)
	...
	{
		pEl.removeAttribute(atrNamePHP);
...
arButtons['Settings'] = ['BXButton',
	{
		...
		{
			this.bNotFocus = true;
			this.pMainObj.OpenEditorDialog("settings", false, 500, {lightMode: lightMode, PHPGetParams: '&light_mode=' + (lightMode ? 'Y' : 'N')});
		}
	}
];
		...
		handler : function ()
			{
				...
				{
					this.bNotFocus = true;
					this.pMainObj.OpenEditorDialog("pasteastext");
				}
			}
	}
];
...
arButtons['table'] = ['BXButton',
	{
		...
		{
			this.bNotFocus = true;
			this.pMainObj.OpenEditorDialog("table", false, 550);
		}
	}
];
...
arButtons['anchor'] = ['BXButton',
	{
		...
		handler: function ()
		{
			...
			this.pMainObj.OpenEditorDialog("anchor", p, 320);
		}
	}
];
...
arButtons['CreateLink'] = ['BXButton',
	{
		...
		handler : function ()
		{
			...
			if (!p || p.tagName != 'A')
				p = false;
			this.pMainObj.OpenEditorDialog("link", p);
		},
		OnSelectionChange: function ()
		{
			var sel = BXGetSelection(this.pMainObj.pEditorDocument, this.pMainObj.pEditorWindow);
			var dis = (BXIsIE() && (sel.type.toLowerCase() == 'none' || sel.createRange().text == '')) || (!BXIsIE() && !sel.type && sel.isCollapsed);
			this.Disable(dis);
		}
	}
];
...
arButtons['image'] = ['BXButton',
	{
		...
		handler : function ()
		{
			...
			if (!p || p.tagName != 'IMG')
				p = false;
			this.pMainObj.OpenEditorDialog("image", p, 440);
		}
	}
];
...
arButtons['SpecialChar'] = ['BXButton',
	{
		...
		{
			this.bNotFocus = true;
			this.pMainObj.OpenEditorDialog("specialchar", false, 600, {pMainObj:this.pMainObj});
		}
	}
];
...
arButtons['tableprop'] = ['BXButton',
	{
		...
		handler: function ()
		{
			...
			if (p) this.pMainObj.OpenEditorDialog("table", p, 500, {check_exists: true});
		}
	}];
...
//arButtons['BackColor'] = ['BXColorPicker1',
arButtons['BackColor'] = ['BXColorPicker',
	{
		id : 'BackColor',
		iconkit : '_global_iconkit.gif',
		...
		OnChange : function (color)
		{
			...
			else
			{
				try{
					...
					if (!color)
						this.pMainObj.pMainObj.executeCommand('removeFormat'); //BXClearMozDirtyInRange(this.pMainObj);
					else
						this.pMainObj.pMainObj.executeCommand('hilitecolor', color);
...
arButtons['ForeColor'] = ['BXColorPicker',
	{
		id : 'ForeColor',
		iconkit : '_global_iconkit.gif',
		...
		{
			if (!color && !BXIsIE())
				this.pMainObj.pMainObj.executeCommand('removeFormat');
			else
...
arButtons['insert_flash'] = [
	'BXButton',
	{
		...
		iconkit : '_global_iconkit.gif',
		name : BX_MESS.FLASH,
		handler : function () {this.bNotFocus = true; this.pMainObj.OpenEditorDialog("flash", false, 500, {pMainObj: this.pMainObj});}
	}
];
...
arButtons['edit_flash'] = [
	'BXButton',
	{
		...
		{
			this.bNotFocus = true;
			this.pMainObj.OpenEditorDialog("flash", false, 500, {pMainObj: this.pMainObj});
		}
	}
];
...
arButtons['_settings'] = ['BXButton', {id : '_settings', iconkit : '_global_iconkit.gif', name : BX_MESS.Settings, title : BX_MESS.Settings_toolbar, handler :function (arParams) {this.pMainObj.OpenEditorDialog("settings", false, 500, {lightMode: lightMode, PHPGetParams: '&light_mode=' + (lightMode ? 'Y' : 'N')});}}];
...
pPropertybarHandlers['a'] = function (bNew, pTaskbar, pElement)
{
	...
	var GAttrEx = function(pEl, atrName, atrNamePHP){return _GAttrEx(pEl, atrName, atrNamePHP,pTaskbar);}
	var SAttrEx = function(pEl,atrName,atrNamePHP,val){return _SAttrEx(pEl,atrName,atrNamePHP,val,pTaskbar);}
	...
	pTaskbar.arElements['href_val'].value = GAttrEx(pElement, '__bxhref', '__bx_php_href');
	pTaskbar.arElements['title'].value = GAttrEx(pElement, 'title', '__bx_php_title');
	...
	var fChange = function ()
	{
		SAttrEx(pElement, '__bxhref', '__bx_php_href', pTaskbar.arElements['href_val'].value);
		SAttrEx(pElement, 'title', '__bx_php_title', pTaskbar.arElements['title'].value);
...
pPropertybarHandlers['anchor'] = function (bNew, pTaskbar, pElement)
{
	...
	var fChange = function (){pElement.setAttribute("__bxcontainer", BXSerialize({"name":pTaskbar.arElements['name'].value}));};
...
pPropertybarHandlers['img'] = function (bNew, pTaskbar, pElement)
{
	...
	pTaskbar.arElements['src'].value = GAttrEx(pElement, '__bxsrc', '__bx_php_src');
	pTaskbar.arElements['alt'].value = GAttrEx(pElement, 'alt', '__bx_php_alt');
	pTaskbar.arElements['title'].value = GAttrEx(pElement, 'title', '__bx_php_title');
	pTaskbar.arElements['cssclass'].SelectByVal(pElement.className);
	pTaskbar.arElements['talign'].SetValue(pElement.align);
	...
	var fChange = function ()
	{
		SAttrEx(pElement, '__bxsrc', '__bx_php_src', pTaskbar.arElements['src'].value);
		SAttrEx(pElement, 'alt', '__bx_php_alt', pTaskbar.arElements['alt'].value);
		SAttrEx(pElement, 'title', '__bx_php_title', pTaskbar.arElements['title'].value);
...
function content_unparse(_node, pMainObj)
{
	...
	else if (!BXIsIE() && _node.arAttributes["__bxtagname"] == 'hr')
		return '<hr />';
...
function _ContentParser(_str, pMainObj)
{
	...
	if (!BXIsIE())
		_str = _str.replace(/<hr([^>]*)>/ig, '<IMG style="padding: 2px; width: 100%; height: 2px;" src="' + image_path + '/break_page.gif" __bxtagname="hr"/>');
	...
	_str = _str.replace(/<embed([^>]*?)>[^>]*?<\/embed>/ig, flash_parser);
...
function Flash_Reload(oPreviewCont, src, height, width)
{
	...
	pFrame.setAttribute("src", flash_preview_path + "?path="+jsUtils.urlencode(src)+"&width="+width+"&height="+height);
}
