...
BXTaskbarSet.prototype.MouseDown = function (e)
{
	switch(this.iNum)
	{
		...
		case 3:
			...
			//var maxVal = val + parseInt(document.getElementById(this.pMainObj.name+'_cEditor').offsetHeight) - 155;
			var d = (BXIsIE()) ? 50 : 155;
			var maxVal = val + parseInt(document.getElementById(this.pMainObj.name+'_cEditor').offsetHeight) - d;
			//var minVal = 160;
			var minVal = 120;
			var bVertical = true;
		...
		case 2:
			...
			var maxVal = val + parseInt(document.getElementById(this.pMainObj.name+'_cEditor').offsetWidth) - 155;
			var minVal = 210;
			var bVertical = false;
			var width = false;
...
BXTaskbarSet.prototype.AddTaskbar = function (pTaskbar, bDontRefresh)
{
	...
	var tbs = this;
	//	setTimeout(function(){tbs.ActivateTaskbar(tbs.arTaskbars[0].id);tbs.Show();}, 3);
	if (this.bShowing)
...
BXTaskbar.prototype.DisplaySingleElement = function (oElement, oCont, orderInd, sPath)
{
	...
	try
	{
		...
		var c = rowTitle.insertCell(-1); //Left cell - icon
		c.unselectable = "on";
		c.style.width = '0%';
		c.className = 'iconcell1';
		...
		_icon.src = oElement.icon || image_path + '/component.gif';
		_icon.ondragstart = function (e) {if(window.event)window.event.cancelBubble = true;};
		_icon.setAttribute("__bxtagname", oElement.tagname);
		_icon.setAttribute("__bxcontainer", BXSerialize(oElement.params));

		if (_this.OnElementClick)
			_icon.onclick = function(e){_this.OnElementClick(this, oElement);};

		if(BXIsIE())
		{
			_icon.onmousedown = function (e){
					this.id = Math.random();
					this.id = _this.pMainObj.nLastDragNDropElement = Math.random();
				};
			_icon.ondragend = function (e){
		...
		c.innerHTML = (oElement.title!='') ? oElement.title : oElement.name;
	...
	{
		_alert('ERROR :: DisplaySingleElement()');
	}
}
...
BXTaskbar.prototype.Display3rdLevelSep = function (oCont,sPath)
{
	...
	_oSeparator.style.width = BXIsIE() ? '80%' : '100%';
	...
	else
	{
		...
		for (var iCh = 0;iCh<oCont.childNodes.length;iCh++)
		{
			try
			{
				...
				{
					_oCont = document.getElementById(this.pref + 'Group_'+arPath[0]);
...
BXTaskbar.prototype.HideGroup = function (arElement, bHide, ilevel)
{
	...
	if (ilevel==0)
	{
		var im_plus = document.getElementById(this.pref + 'Group_plus_'+arElement.name);
		var elementsGroup = document.getElementById(this.pref + 'Group_'+arElement.name);
		if(!bHide)
		{
	...
	{
		var plusIcon = document.getElementById(this.pref + 'Plus_1_icon_'+arElement.name);
		var groupIcon = document.getElementById(this.pref + 'Folder_1_icon_'+arElement.name);
		var elementsGroup1 = document.getElementById(this.pref + 'Group_1_'+arElement.name);
		if(!bHide)
		{
			arElement.hidden = false;
...
function BXPropertiesTaskbar()
{
	...
	BXPropertiesTaskbar.prototype.OnTaskbarCreate = function ()
	{
		...
		this.pCellProps.innerHTML = "<br /><span style='padding-left: 15px;'>" + BX_MESS.SelectAnyElement + "</span>";
...
function cleanPropPanelLeak()
{
	return;
	var prev_el = ar_PROP_ELEMENTS.length-1;
	if (prev_el>=0)
	{
		var pTaskbar = ar_PROP_ELEMENTS[prev_el];
		if (pTaskbar && pTaskbar.OnClean) pTaskbar.OnClean();
		try
		{
			pTaskbar.parentCell = pTaskbar.oOldSelected = pTaskbar.parentCell = pTaskbar.pTaskbarSet = pTaskbar.oOldPropertyPanelElement = pTaskbar.pWnd = pTaskbar.pTitleRow = null;
		}
		catch (e) {}
	}
}
...
function BXCreateTaskbars(pMainObj, bDrawTabs)
{
	...
	for(i = 0, l = pMainObj.arTaskbarSet.length; i < l; i++)
	{
		...
		if (!tbs.arTaskbars[0])
			continue;
		//	setTimeout(function(){tbs.ActivateTaskbar(tbs.arTaskbars[0].id);}, 5);
		if (l1 > 1)
...
{
	this.pMainObj = pMainObj;
	this.oCont = document.getElementById(this.pMainObj.name+'_toolBarSet0');
