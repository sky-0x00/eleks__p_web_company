...
if ((isset($_POST['id_type'])) && (!empty($_POST['id_type'])) && (is_numeric($_POST['id_type'])) && 
	...
	$response = $Objects -> CreateObject ($_POST['id_type'], utf8($_POST['name']), utf8($_POST['town']), utf8($_POST['short_description']), utf8($_POST['description']), $_POST['primary']);
	
	if (($response['result']=="success") && (isset($_POST['picture'])) && (!empty($_POST['picture']))) {
		
		if (!@file_exists($Objects->path_server . $response['text'] . "/")) {
			if (@mkdir($Objects->path_server . $response['text'] . "/", 0777))
				@chmod($Objects->path_server . $response['text'] . "/", 0777);
		}
		
		$namearray = explode(".", $_POST['picture']);
						
		$filename = $Objects->path_server . $response['text'] . "/" . $response['text'] . "." . $namearray[count($namearray)-1];
